...
- optional `--max-actions` safety fuse for debugging runaway loops (omitted by default, so matches run until game over)


### Docker Compose service (Option B)

`docker compose up` now also starts a dedicated `sim-harness-web` service that serves the dashboard on port `8765`.

- Main Apache app: `http://localhost:8080/Arena/MainMenu.php`
- Simulation dashboard: `http://localhost:8765`

You can still run the UI manually if needed:
